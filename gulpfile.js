...
const globalProtos = "global-prototypes.js";
...
const espConsoleBeingWatchedFilePath = path.join(
  distDir,
  espConsoleBeingWatchedFileName
);
...
function compile_ts() {
  ...
  return tsResult.js
    .pipe(gulpIf([globalProtos, appFileName], gulpConcat(appFileName)))
    .pipe(gulp.dest(distDir));
}

function content_to_dist() {
  return gulp.src("src/**/*.js", { base: "src" }).pipe(gulp.dest(distDir));
}

function send_to_espruino_console(cb) {
  console.log(`espReadyBundlePath`, espReadyBundlePath);
  const content = fs.readFileSync(espReadyBundlePath);
  fs.writeFile(espConsoleBeingWatchedFilePath, content, (err) => {
    if (err) {
      throw err;
    }
    cb();
...
function clear_espruino_watch_file(cb) {
  fs.writeFile(espConsoleBeingWatchedFilePath, "", (err) => {
    if (err) {
      throw err;
    }
    cb();
...
function gen_config_ts(cb) {
  if (!fs.existsSync(userAppConfigFileName)) {
    const content = fs
      .readFileSync(appConfigFileName)
      .toString()
      .split("\n")
      .map((x) => `# ${x}`)
      .join("\n");
  ...
  fs.writeFileSync(
    path.join(srcDir, appConfigTsFileName),
    resultConfigTsContent
  );
  cb();
}
...
gulp.task(
  ...
  gulp.series("compile-ts", "content-to-dist", prepare_for_espruino)
);
